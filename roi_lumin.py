...
#    grey = cv.CreateImage((width, height), cv.IPL_DEPTH_8U, 1)
    ...
    while True:
        ...
        cv.Threshold(grey, grey, 230, 1, cv.CV_THRESH_BINARY)
        moments = cv.Moments(grey)
        a = cv.GetSpatialMoment(moments, 0, 0)
        area.append(a)
        ...
        ibrightness.append(avg)

        for i, roi in enumerate(job_rois):
            cv.SetImageROI(grey, tuple(roi))
            avg = cv.Avg(grey)[0]
            brightness[i].append(avg)
        frame_num += 1
...
#    fps = float(job['fps'])
    ...
    plt.figure()
    plt.plot(vid_time, ibrightness, label='integral')
    plt.plot(vid_time, np.sqrt(area), label='area')
    for i, _ in enumerate(job_rois):
        plt.plot(vid_time, brightness[i], label='%d' % i)
    title = infn.replace('_', r'\_')
    plt.title(title)
    plt.legend()
    pngfn = 'roi_bri_' + infn[:-4] + '.png'
    plt.savefig(pngfn)
    ...
    data = np.empty((len(ibrightness), 3 + len(brightness)))
    data[:, 0] = vid_time
    data[:, 1] = ibrightness
    data[:, 2] = area
